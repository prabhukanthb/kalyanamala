...
const profileValidation = [
  ...
  body('currentAddress.pinCode').matches(/^[0-9]{6}$/).withMessage('Pin code must be 6 digits'),

  body('presentAddress.streetName').notEmpty().withMessage('Present street name is required'),
  body('presentAddress.city').notEmpty().withMessage('Present city is required'),
  body('presentAddress.state').notEmpty().withMessage('Present state is required'),
...
function buildProfilePayload(bodyData, reqUserId, isAdminCreate = false) {
  const profilePayload = {
    ...
    presentAddress: {
      streetName: bodyData.presentAddress?.streetName,
      city: bodyData.presentAddress?.city,
      state: bodyData.presentAddress?.state,
      country: bodyData.presentAddress?.country || 'India',
      pinCode: bodyData.presentAddress?.pinCode
    },
    ...
    partnerRequirement: bodyData.partnerRequirement,
...
router.put('/users/:userId', authMiddleware, adminMiddleware, userIdValidation, [
  ...
  try {
    ...
    const { email, phone, alternativePhone, firstName, lastName, surname, role, status } = req.body;
    ...
    if (alternativePhone !== undefined) user.alternativePhone = alternativePhone || null;
...
router.get('/profiles', authMiddleware, adminMiddleware, listProfilesValidation, async (req, res) => {
  try {
    ...
    const profiles = await Profile.find(query)
      .populate('userId', 'firstName lastName surname email phone alternativePhone role status')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit, 10))
      .lean();
