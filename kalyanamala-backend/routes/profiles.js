...
const profileValidation = [
  ...
  body('presentAddress.streetName')
    .trim()
    .notEmpty()
    .withMessage('Present street name is required'),

  body('presentAddress.city')
    .trim()
    .notEmpty()
...
router.put('/:id', authMiddleware, requireRole('admin', 'subadmin'), async (req, res) => {
  try {
    ...
    if (req.body.presentAddress) {
      profile.presentAddress = {
        streetName: safeString(req.body.presentAddress.streetName ?? profile.presentAddress?.streetName),
        city: safeString(req.body.presentAddress.city ?? profile.presentAddress?.city),
        state: safeString(req.body.presentAddress.state ?? profile.presentAddress?.state),
